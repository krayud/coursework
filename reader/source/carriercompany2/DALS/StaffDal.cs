...
namespace carriercompany2.DALS
{
    class StaffDal
    {
        ...
        {
            dal.SetQuery(@"SELECT drivers.name as [Водитель], brigades.id as [ID бригады], brigadiers.name as [Бригадир] FROM brigadiers, drivers, brigades
        ...
        {
            dal.SetQuery(@"SELECT  workers.name as [Рабочий], workshops.id as [ID цеха], workshops.name as [Название цеха], masters.name as [Мастер]
